...
void applyConfig(uint8_t resolutionId, uint8_t fps, uint8_t quality) {
    ...
    if (fps > 0 && fps <= 30) {
        ...
        Serial.printf("[CFG] Frame interval set to %d ms (~%d fps)\n",
...
void loop() {
    ...
    // ── Capture & transmit ────────────────────────────────────────────────────
    if (now - lastFrameMs >= g_frameIntervalMs) {   // <-- changed to use handshake interval
        lastFrameMs = now;
    ...
    // Small yield to keep RTOS happy
    delay(1);
}